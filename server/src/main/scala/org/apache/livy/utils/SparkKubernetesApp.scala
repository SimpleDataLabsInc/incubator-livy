...
class SparkKubernetesApp private[utils] (
  ...
  private[utils] val kubernetesAppMonitorThread = Utils
    .startDaemonThread(s"kubernetesAppMonitorThread-$this") {
      try {
        val app =
          try getAppFromTag(appTag, pollInterval, appLookupTimeout.fromNow)
          catch {
            case e: Exception ⇒
        ...
        debug(
          s"Application $appId is in state $state\nDiagnostics:" +
      ...
      } finally info(s"Finished monitoring application $appTag with state $state")
    }
  ...
  override def kill(): Unit = {
    ...
    } finally process.foreach(_.destroy())
  }
  ...
  /**
    ...
    */
  @tailrec
  private def getAppFromTag(appTag: String, pollInterval: ScalaDuration, deadline: Deadline): KubernetesApplication = {

    withRetry(kubernetesClient.getApplications().find(_.getApplicationTag.contains(appTag))) match {
      case Some(app) ⇒ app
      case None ⇒
        if (deadline.isOverdue) {
          ...
          throw new IllegalStateException(
            "No Kubernetes application is found with tag" +
...
private[utils] case class KubernetesAppReport(driver: Option[Pod], executors: Seq[Pod], appLog: IndexedSeq[String])
    extends Logging {

  def getApplicationState: String = {
    driver match {
      case Some(d) ⇒
        val status          = d.getStatus
...
private[utils] class LivyKubernetesClient(client: DefaultKubernetesClient, namespaces: Set[String] = Set.empty) {
    ...
    Option(namespaces)
      ...
      .map(
        _.pods
          ...
          .getItems
          .asScala
          .map(new KubernetesApplication(_))
      )
      .reduce(_ ++ _)
  }
  ...
  private def getApplicationLog(app: KubernetesApplication, cacheLogSize: Int): IndexedSeq[String] = {
    Try(
      client
        .inNamespace(app.getApplicationNamespace)
        .pods
        .withName(app.getApplicationPod.getMetadata.getName)
        .tailingLines(cacheLogSize)
        .getLog
        .split("\n")
        .toIndexedSeq
...
private[utils] object KubernetesClientFactory {
  ...
  def createKubernetesClient(livyConf: LivyConf): LivyKubernetesClient = {
    ...
    val config = new ConfigBuilder()
      ...
      .withOption(oauthTokenValue) { (token, builder) ⇒
        builder.withOauthToken(token)
      }
      .withOption(oauthTokenFile) { (file, builder) ⇒
        builder.withOauthToken(Files.toString(new File(file), Charsets.UTF_8))
      }
      .withOption(caCertFile) { (file, builder) ⇒
        builder.withCaCertFile(file)
      }
      .withOption(clientKeyFile) { (file, builder) ⇒
        builder.withClientKeyFile(file)
      }
      .withOption(clientCertFile) { (file, builder) ⇒
        builder.withClientCertFile(file)
      }
      .withOption(clientNamespace) { (namespace, builder) ⇒
        builder.withNamespace(namespace)
      }
      .build()
    new LivyKubernetesClient(new DefaultKubernetesClient(config), livyConf.getKubernetesNamespaces())
  }
  ...
  implicit private class OptionConfigurableConfigBuilder(val configBuilder: ConfigBuilder) extends AnyVal {

    def withOption[T](option: Option[T])(configurator: (T, ConfigBuilder) ⇒ ConfigBuilder): ConfigBuilder =
